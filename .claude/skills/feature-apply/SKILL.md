...
2. 承認が得られたら、まず `feat/<slug>` ブランチを切る（**main には直接コミットしない**）:
   ...
   - 問題なければ:
     ...
     ```
3. サンドボックスの内容を、この `feat/<slug>` ブランチのワーキングツリーに反映する:
   ```bash
...
6. 通ったら `feat/<slug>` ブランチにローカルコミットする（監査証跡・ロールバック用。
...
7. コミットできたら `main` に戻り、サンドボックスを削除する:
   ...
   ```
8. `docs/specs/<slug>.md` も apply により `feat/<slug>` 側へコピーされ、6でコミット済みのはず。
   ユーザーに完了を報告する（ブランチ名・コミットハッシュを含める）。続けて push するか確認する:
   - push する場合は `git push -u origin feat/<slug>`。
   - PRも作る場合は `gh pr create --base main --head feat/<slug>` を提案する（CIが起動する）。
   - どちらも明示的な承認を得てから実行する。
